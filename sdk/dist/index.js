...
// src/types/index.ts
var ZkProofOutputSchema = zod.z.object({
...
var ZkProofResultSchema = zod.z.object({
...
var MultiSigConfigSchema = zod.z.object({
...
var BatchTransactionSchema = zod.z.object({
...
function isValidMnemonic(mnemonic) {
  try {
    ethers.ethers.Mnemonic.fromPhrase(mnemonic);
    return true;
  } catch {
    return false;
  }
...
function formatEther(wei, decimals = 4) {
  return parseFloat(ethers.ethers.formatEther(wei)).toFixed(decimals);
}
function formatGwei(wei, decimals = 2) {
  return parseFloat(ethers.ethers.formatUnits(wei, "gwei")).toFixed(decimals);
}
...
function generatePrivateKey() {
  return ethers.ethers.Wallet.createRandom().privateKey;
}
...
function publicKeyToAddress(publicKey) {
  return ethers.ethers.computeAddress(publicKey);
}
...
function recoverAddress(message, signature) {
  return ethers.ethers.verifyMessage(message, signature);
}
function hashSHA256(data) {
  return sha256.sha256(data);
}
function hashKeccak256(data) {
  return ethers.ethers.keccak256(data);
}
function hashString(str) {
  return ethers.ethers.id(str);
}
...
function hashTypedData(domain, types, value) {
  return ethers.ethers.TypedDataEncoder.hash(domain, types, value);
}
...
function verifyTypedData(domain, types, value, signature, address) {
  try {
    const recoveredAddress = ethers.ethers.verifyTypedData(domain, types, value, signature);
    return recoveredAddress.toLowerCase() === address.toLowerCase();
  } catch {
    return false;
  }
...
function generateSecureSalt() {
  ...
  return BigInt(ethers.ethers.hexlify(randomBytes));
}
...
function createWalletFromMnemonic(mnemonic, path = "m/44'/60'/0'/0/0") {
  ...
  return new ethers.ethers.Wallet(hdNode.privateKey);
}
...
async function decryptPrivateKey(encryptedKey, password) {
  const wallet = await ethers.ethers.Wallet.fromEncryptedJson(encryptedKey, password);
  return wallet.privateKey;
}
function generateDeterministicAddress(factory, salt, initCodeHash) {
  const packed = ethers.ethers.solidityPacked(
...
function splitSignature(signature) {
  return ethers.ethers.Signature.from(signature);
}
function joinSignature(r, s, v) {
  const signature = ethers.ethers.Signature.from({ r, s, v });
...
function getUserOpHash(userOp, entryPoint, chainId) {
  ...
  const finalPacked = ethers.ethers.solidityPacked(
...
var GuardianAA = class {
  ...
  async initialize(signer) {
    try {
      this.provider = new ethers.ethers.JsonRpcProvider(this.config.network.rpcUrl);
      if (signer) {
        this.signer = signer.connect(this.provider);
      }
...
exports.BatchTransactionSchema = BatchTransactionSchema;
...
exports.DEFAULT_GAS_LIMITS = DEFAULT_GAS_LIMITS;
exports.DEV_CONSTANTS = DEV_CONSTANTS;
exports.ENTRYPOINT_ADDRESS_V06 = ENTRYPOINT_ADDRESS_V06;
...
exports.GAS_PRICES = GAS_PRICES;
exports.GasEstimationSchema = GasEstimationSchema;
exports.GuardianAA = GuardianAA;
exports.GuardianAAError = GuardianAAError;
...
exports.MultiSigConfigSchema = MultiSigConfigSchema;
exports.NetworkConfigSchema = NetworkConfigSchema;
exports.PaymasterConfigSchema = PaymasterConfigSchema;
...
exports.UTILS_CONTRACT_ADDRESSES = UTILS_CONTRACT_ADDRESSES;
exports.UTILS_ZK_CONSTANTS = UTILS_ZK_CONSTANTS;
...
exports.ZkProofConfigSchema = ZkProofConfigSchema;
exports.ZkProofError = ZkProofError;
exports.ZkProofOutputSchema = ZkProofOutputSchema;
exports.ZkProofResultSchema = ZkProofResultSchema;
exports.ZkProver = ZkProver;
exports.ZkVerifier = ZkVerifier;
exports.areValidUniqueAddresses = areValidUniqueAddresses;
exports.createGuardianAA = createGuardianAA;
exports.createLocalGuardianAA = createLocalGuardianAA;
exports.createSepoliaGuardianAA = createSepoliaGuardianAA;
...
exports.formatHexString = formatHexString;
...
exports.generateDeterministicAddress = generateDeterministicAddress;
...
exports.isValidHexLength = isValidHexLength;
...
exports.isValidUserOperation = isValidUserOperation;
...
exports.validateAccountDeployment = validateAccountDeployment;
